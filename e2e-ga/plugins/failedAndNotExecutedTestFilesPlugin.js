...
const resultsDir = process.env.GA_TEST_RESULTS_DIR
  || process.env.REPORT_DIR
  || 'test-results/functional-ga';

const tempFailedTestsPath = `${resultsDir}/tempFailedTests.json`;
const tempPassedTestsPath = `${resultsDir}/tempPassedTests.json`;
const tempToBeExecutedTestFilesPath = `${resultsDir}/tempToBeExecutedTestFiles.json`;
const testFilesReportPath = `${resultsDir}/testFilesReport.json`;
...
async function addTestToFile(testToAdd, testsPath) {
  ...
  try {
    ...
    const exists = tests.some((test) => test.testId === testToAdd.uuid && test.testFile === normalisedTestFile);
    if (!exists) {
