...
const pipelineTagMap = {
  '@ui-prod': [
    'civil-general-apps-ccd-definition: master',
    'civil-general-apps-ccd-definition: PR',
    'civil-general-apps-ccd-definition: nightly'
  ],
...
const actorStepObjects = ['I', 'api', 'api_sdo', 'wa'];
const ignoredMethodPrefixes = ['verify'];
...
function normaliseTag(token) {
  ...
  if (!trimmed.startsWith('@')) {
    if (trimmed.startsWith('ui-') || trimmed.startsWith('api-') || pipelineTagSet.has(`@${trimmed}`)) {
      trimmed = `@${trimmed}`;
    }
  }
  return trimmed;
...
function extractHelperSteps(fn) {
  ...
  const matches = [];
  const hasIgnoredMethodPrefix = method => {
    const lower = method.toLowerCase();
    return ignoredMethodPrefixes.some(prefix =>
  ...
  while ((match = stepsRegex.exec(source))) {
    if (
      ...
      !hasIgnoredMethodPrefix(match[2])
    ) {
  ...
  while ((match = verifyObjectRegex.exec(source))) {
    if (!hasIgnoredMethodPrefix(match[2])) {
      matches.push({ name: `${match[1]}.${match[2]}`, index: match.index });
  ...
  if (standaloneStepPrefixes.length) {
    ...
    while ((match = standaloneRegex.exec(source))) {
      ...
      if (
        ...
        !hasIgnoredMethodPrefix(methodName)
      ) {
