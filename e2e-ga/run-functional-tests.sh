...
RESULTS_DIR="${GA_TEST_RESULTS_DIR:-test-results/functional-ga}"
...
run_functional_test_groups() {
  command="yarn ga:test:ui-nonprod --grep "
...
run_functional_tests() {
  ...
  if [ "$ENVIRONMENT" = "aat" ]; then
    yarn ga:test:ui-prod
  elif [ -z "$PR_FT_GROUPS" ]; then
    yarn ga:test:ui-nonprod 
  else
    run_functional_test_groups
  fi
}
...
#MAIN SCRIPT
TEST_FILES_REPORT="${RESULTS_DIR}/testFilesReport.json"
PREV_TEST_FILES_REPORT="${RESULTS_DIR}/prevTestFilesReport.json"
