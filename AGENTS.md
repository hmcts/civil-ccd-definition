...
- For civil-ccd-definition (this repo):
  ...
  * For the master and PR pipeline there is a 'Smoke Test' stage where smoke tests are run. The tests are run using yarn and the script that is runs, defined in the package.json , is test:smoke this then runs a command called test:civil-ccd-smoke for both the master and PR pipelines, the tag that is used for both pipelines is @civil-ccd-smoke.
...
- For civil-service:
  ...
  * The projects in those repos are gradle projects.
  * For the master and PR pipelines there is a 'Smoke Test' stage which runs a gradle task called 'smoke', this then runs a yarn command and the script that is run, defined in the package.json file, called 'test:civil-service-smoke' for master and for PR pipelines which looks for tests in a set of files defined in the './codecept.conf.js' file which have the following tags '@civil-service-smoke' for the master pipeline and the preview pipelines.
  * For the master and PR pipeline there is a 'Functional Test' stage, which runs a gradle task called 'functional', this then executes a similar shell script to the one executed by 'test:functional', which in turn runs a yarn command and the script that is run, defined in the package.json file, called 'test:civil-service-master' for master and 'test:civil-service-pr' for PR. which looks for tests in a set of files defined in the './codecept.conf.js' file which have the following tags '@civil-service-master' for the master pipeline and '@civil-service-pr' for the preview pipeline.
...
- For civil-camunda-bpmn-definition:
  ...
  * The pipeline pulls in the latest master version of this repo with all its e2e tests into it's workspace.
  * The projects in those repos are gradle projects.
  * For the master and PR pipelines there is a 'Smoke Test' stage which runs a gradle task called 'smoke', this then runs a yarn command and the script that is run, defined in the package.json file, called 'test:civil-camunda-smoke' for master and for PR pipelines which looks for tests in a set of files defined in the './codecept.conf.js' file which have the following tags '@civil-camunda-smoke' for the master pipeline and the preview pipelines.
...
- For the civil-wa-task-configuration repo:
  ...
  * The pipeline pulls in the latest master version of this repo with all its e2e tests into it's workspace.
  * The projects in those repos are gradle projects.
  * For the master and PR pipelines there is a 'Smoke Test' stage which runs a gradle task called 'smoke', this then runs a yarn command and the script that is run, defined in the package.json file, called 'test:civil-wa-smoke' for master and for PR pipelines which looks for tests in a set of files defined in the './codecept.conf.js' file which have the following tags '@civil-wa-smoke' for the master pipeline and the preview pipelines.
