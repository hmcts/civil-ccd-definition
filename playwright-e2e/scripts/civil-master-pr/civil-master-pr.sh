...
run_functional_test_groups() {
  command="yarn $PLAYWRIGHT_PR_COMMAND --grep "
...
run_failed_functional_tests() {
  ...
  if [ "$ENVIRONMENT" = "aat" ]; then
    if ! yarn "$PLAYWRIGHT_MASTER_COMMAND" --last-failed; then
      run_playwright_teardown
      exit 1
    fi
  else
    if ! yarn "$PLAYWRIGHT_PR_COMMAND" --last-failed; then
      run_playwright_teardown
      exit 1
    fi
  fi
...
run_functional_tests() {
  ...
  if [ "$ENVIRONMENT" = "aat" ]; then
    if ! yarn "$PLAYWRIGHT_MASTER_COMMAND"; then
      run_playwright_teardown
      exit 1
    fi
  elif [ -z "$PLAYWRIGHT_PR_FT_GROUPS" ]; then
    if ! yarn "$PLAYWRIGHT_PR_COMMAND"; then
      run_playwright_teardown
      exit 1
    fi
  else
