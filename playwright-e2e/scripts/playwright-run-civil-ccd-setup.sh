if [ "$RUN_FAILED_TESTS" = "true" ]; then
  ...
  PREV_PLAYWRIGHT_TEST_FILES_REPORT="playwright-functional-results/prevPlaywrightTestFilesReport.json"
  ...
  cp -f "$PLAYWRIGHT_TEST_FILES_REPORT" "$PREV_PLAYWRIGHT_TEST_FILES_REPORT"
  cp -f "$PLAYWRIGHT_LAST_RUN_REPORT" "$PREV_PLAYWRIGHT_LAST_RUN_REPORT"
