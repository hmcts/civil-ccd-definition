...
test('Testing Login', async ({ IdamSteps, ExuiDashboardSteps, ApiUsersSteps, _requestsFactory, _exuiDashboardPageFactory, _informAgreedExtensionDateFactory }) => {
  await IdamSteps.DefendantSolicitor1Login();
  await ExuiDashboardSteps.GoToCaseList();
  await ApiUsersSteps.SetupUserData(civilAdminUser);
