...
Scenario.skip('2v1 claimant claimant upload mediation docs @carm @non-prod-e2e-ft', async ({LRspec}) => {
  await LRspec.login(config.applicantSolicitorUser);
  await LRspec.uploadMediationDocs(civilCaseReference, 'Both Claimants', 'Both docs');
}).retry(3);

Scenario.skip('2v1 claimant defendant upload mediation docs @carm @non-prod-e2e-ft', async ({LRspec}) => {
  await LRspec.login(config.defendantSolicitorUser);
  await LRspec.uploadMediationDocs(civilCaseReference, 'Defendant 1', 'Non-attendance');
}).retry(3);

Scenario.skip('1v2 upload mediation documents in different SDO states @carm @e2e-nightly-nonprod', async ({api_spec, LRspec}) => {
