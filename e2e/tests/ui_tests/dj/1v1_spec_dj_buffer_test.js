...
// Known gaps tracked separately (not covered by this file):
// - DTSCCI-5327 AC1 (EXUI confirmation Welsh): EXUI Welsh requires setting claimantLanguagePreferenceDisplay
//   on the case before DJ submit; out of scope for this LR-vs-LiP English-default fixture.
// - DTSCCI-5327 AC1 EN: confirmation header is shipped as 'Default judgment requested' which diverges from
//   the AC text 'A CCJ has been requested...' - dev defect raised, this test asserts shipped header until fixed.
...
Scenario('01 Create LRvLiP spec claim, LR claimant requests DJ - case parks in JUDGMENT_REQUESTED with buffer confirmation (DTSCCI-5327 AC1, DTSCCI-5101 AC1)', async ({I, api_spec_cui}) => {
  ...
  // TODO update expected header to AC text 'A CCJ has been requested. You will be notified when this is confirmed' once dev fixes DTSCCI-5327 wording defect.
  ...
  // DTSCCI-3661 AC1 explicit assertion: 'no judgment is issued or registered yet'
...
Scenario('05 Scheduler issues buffered DJ once 48h+ buffer expires - full end-to-end (DTSCCI-5436 positive)', async ({I, api_spec_cui}) => {
  ...
  await new Promise(resolve => setTimeout(resolve, 5000)); // allow ES indexing
...
Scenario('08 CTSC takes case offline during buffer - case proceeds offline to PROCEEDS_IN_HERITAGE_SYSTEM (DTSCCI-3661 AC4)', async ({I, api_spec_cui}) => {
  ...
  // AC4: ops take the case offline via case-proceeds-offline event during buffer
  await I.login(config.adminUser);
  await I.caseProceedsInCaseman(offlineCaseId);
  await api_spec_cui.waitForBusinessProcessFinished();

  // AC4 only requires the case to proceed offline; activeJudgment cancellation is not part of AC4.
