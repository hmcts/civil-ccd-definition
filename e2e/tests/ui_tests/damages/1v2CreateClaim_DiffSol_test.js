...
Scenario('Claimant solicitor raises a claim against 2 defendants who have different solicitors', async ({I, loginAs }) => {
...
Scenario('Claimant solicitor notifies both defendants of claim', async ({I, loginAs}) => {
  await loginAs('organisation1Solicitor1');
...
Scenario('Claimant solicitor notifies defendant solicitors of claim details', async ({I, loginAs}) => {
  await loginAs('organisation1Solicitor1');
...
*/
Scenario('Defendant 1 solicitor requests deadline extension', async ({I, loginAs}) => {
...
Scenario('Defendant 1 solicitor adds defendant litigation friend', async ({I, loginAs}) => {
  await loginAs('organisation2Solicitor1');
...
Scenario('Defendant 1 solicitor rejects claim for defendant 1', async ({I, loginAs}) => {
  await loginAs('organisation2Solicitor1');
...
Scenario('Defendant 2 solicitor rejects claim for defendant 2', async ({I, loginAs}) => {
  await loginAs('organisation3Solicitor1');
  await I.respondToClaim({
...
Scenario('Claimant solicitor responds to defence', async ({I, loginAs}) => {
  await loginAs('organisation1Solicitor1');
...
Scenario('Judge triggers SDO', async ({I, loginAs}) => {
...
Scenario('Claimant solicitor uploads evidence', async ({I, loginAs}) => {
...
Scenario('Defendant solicitor uploads evidence', async ({I, loginAs}) => {
  if (['preview', 'demo'].includes(config.runningEnv)) {
    await loginAs('organisation2Solicitor1');
