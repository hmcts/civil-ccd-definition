...
Scenario('01 1v1 spec full defence multi claim', async ({api_spec}) => {
...
Scenario('02 1v1 spec full admission multi claim', async ({api_spec}) => {
  const mpScenario = 'ONE_V_ONE';
  await api_spec.createClaimWithRepresentedRespondent(config.applicantSolicitorUser, mpScenario, false, true, claimAmountPenniesMulti);
...
Scenario.skip('03 1v1 spec part admission multi claim', async ({api_spec}) => {
  const mpScenario = 'ONE_V_ONE';
  await api_spec.createClaimWithRepresentedRespondent(config.applicantSolicitorUser, mpScenario, false, true, claimAmountPenniesMulti);
...
Scenario('04 1v1 spec counter claim multi claim', async ({api_spec}) => {
  const mpScenario = 'ONE_V_ONE';
  await api_spec.createClaimWithRepresentedRespondent(config.applicantSolicitorUser, mpScenario, false, true, claimAmountPenniesMulti);
...
Scenario('05 1v2DS spec full defence multi claim', async ({api_spec}) => {
...
Scenario('06 1v2SS spec full defence multi claim', async ({I, api_spec}) => {
