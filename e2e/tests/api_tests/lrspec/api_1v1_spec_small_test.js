...
Feature('1v1 spec api small claims journeys').tag('@api-spec-small @api-nightly-prod');

Scenario('01 1v1 FULL_DEFENCE claimant and defendant response small claim', async ({I, api_spec_small}) => {
...
Scenario('02 1v1 FULL_ADMISSION claimant and defendant response small claim', async ({I, api_spec_small}) => {
  await api_spec_small.createClaimWithRepresentedRespondent(config.applicantSolicitorUser);
...
Scenario('03 1v1 PART_ADMISSION claimant and defendant response small claim', async ({I, api_spec_small}) => {
  await api_spec_small.createClaimWithRepresentedRespondent(config.applicantSolicitorUser);
  await api_spec_small.defendantResponse(config.defendantSolicitorUser, 'PART_ADMISSION');
  await api_spec_small.claimantResponse(config.applicantSolicitorUser);
});

Scenario('04 1v1 COUNTER_CLAIM claimant and defendant response small claim', async ({I, api_spec_small}) => {
  await api_spec_small.createClaimWithRepresentedRespondent(config.applicantSolicitorUser);
...
Scenario('05 1v1 FULL_DEFENCE claimant and defendant response small claim - CARM enabled', async ({I, api_spec_small}) => {
