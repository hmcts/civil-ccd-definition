...
Scenario('1v2 small claim full defence, defendant response', async ({I, api_spec}) => {
  ...
  await api_spec.createCaseFlags(config.hearingCenterAdminWithRegionId4);
  await api_spec.manageCaseFlags(config.hearingCenterAdminWithRegionId4);
});
...
Scenario('1v2 small claim full defence, claimant response @api-prod-specified', async ({I, api_spec}) => {
  await api_spec.createClaimWithRepresentedRespondent(config.applicantSolicitorUser, 'ONE_V_TWO_SAME_SOL');
  await api_spec.defendantResponse(config.defendantSolicitorUser, 'FULL_DEFENCE', 'ONE_V_TWO');
