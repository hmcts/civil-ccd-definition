...
Scenario('06 1v1 full admit claimant and defendant response @api-spec-full-admit', async ({I, api_spec}) => {
...
Scenario('07 1v1 part admit defence claimant and defendant response @api-spec-part-admit', async ({I, api_spec}) => {
...
Scenario('08 1v1 Settle claim - full defence claimant and defendant response', async ({I, api_spec}) => {
  await api_spec.createClaimWithRepresentedRespondent(config.applicantSolicitorUser);
  await api_spec.informAgreedExtensionDate(config.applicantSolicitorUser);
  await api_spec.defendantResponse(config.defendantSolicitorUser);
  await api_spec.claimantResponse(config.applicantSolicitorUser, 'FULL_DEFENCE', 'ONE_V_ONE',
