...
async function respondWithPAPayByInstallmentsJBAPaidInFull(api_spec_cui) {
  ...
  await api_spec_cui.performMarkPainInFull(config.applicantCitizenUser, caseId);
}

async function respondWithFAPayBySetDateJBAPaidInFull(api_spec) {
...
Scenario.only('1v1 LiP v LiP defendant response with part admit pay by installments judgment by admission mark paid in full', async ({api_spec_cui}) => {
  await respondWithPAPayByInstallmentsJBAPaidInFull(api_spec_cui);
});
