...
Feature('Request for reconsideration - 1v1 - spec @api-nonprod @api-specified @api-nightly-prod @api-r2-sdo');
...
Scenario('1v1 spec request for reconsideration for uphold previous order', async ({api_spec_small}) => {
    ...
    await api_spec_small.judgeDecisionOnReconsiderationRequest(judgeUserReg1, 'YES');
});
...
Scenario('1v1 spec request for reconsideration for create new SDO', async ({api_spec_small}) => {
    ...
    await api_spec_small.createSDO(judgeUserReg1, 'CREATE_SMALL_NO_SUM');
});
