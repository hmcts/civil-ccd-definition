...
Feature('Request for reconsideration - 1v1 - spec').tag('@api-nightly-prod @api-sdo'); // reinstate @api-nightly-prod tag when issue described on CIV-14871 is resolved

Scenario('01 1v1 spec request for reconsideration for uphold previous order', async ({api_spec_small}) => {
...
Scenario('02 1v1 spec request for reconsideration for create new SDO', async ({api_spec_small}) => {
    await api_spec_small.createClaimWithRepresentedRespondent(config.applicantSolicitorUser, 'ONE_V_ONE', false, false);
    await api_spec_small.defendantResponse(config.defendantSolicitorUser, 'FULL_DEFENCE', 'ONE_V_ONE', true);
    await api_spec_small.claimantResponse(config.applicantSolicitorUser, true);
    await api_spec_small.createSDO(legalAdvUser, 'CREATE_SMALL_NO_SUM');
...
Scenario('03 1v1 spec request for reconsideration for create general order', async ({api_spec_small}) => {
    await api_spec_small.createClaimWithRepresentedRespondent(config.applicantSolicitorUser, 'ONE_V_ONE', false, false);
    await api_spec_small.defendantResponse(config.defendantSolicitorUser, 'FULL_DEFENCE', 'ONE_V_ONE', true);
    await api_spec_small.claimantResponse(config.applicantSolicitorUser, true);
    await api_spec_small.createSDO(legalAdvUser, 'CREATE_SMALL_NO_SUM');
...
Scenario('04 1v2 spec request for reconsideration by defendant2 for create general order', async ({api_spec_small}) => {
...
Scenario.skip('05 1v1 spec request for reconsideration when claim amount is greater than 1000', async ({api_spec}) => {
...
Scenario('06 1v1 spec request for reconsideration for create a new SDO', async ({api_spec_small}) => {
    await api_spec_small.createClaimWithRepresentedRespondent(config.applicantSolicitorUser, 'ONE_V_ONE', false, false);
    await api_spec_small.defendantResponse(config.defendantSolicitorUser, 'FULL_DEFENCE', 'ONE_V_ONE', true);
    await api_spec_small.claimantResponse(config.applicantSolicitorUser, true);
    await api_spec_small.createSDO(legalAdvUser, 'CREATE_SMALL_NO_SUM');
