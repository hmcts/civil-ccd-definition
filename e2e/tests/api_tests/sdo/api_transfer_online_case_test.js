...
const judgeUser = config.judgeUserWithRegionId1Local; //TODO change to other user after testing locally
...
Scenario('Transfer Online Spec claim 1v2 - judge user - not suitable SDO - Transfer Case', async ({I, api_spec}) => {
  if (['preview', 'demo'].includes(config.runningEnv)) {
    console.log('--createClaimWithRepresentedRespondent--');
    ...
    await api_spec.claimantResponse(config.applicantSolicitorUser, 'FULL_DEFENCE', 'ONE_V_TWO',
      'JUDICIAL_REFERRAL');
    console.log('notSuitableForSdo-transferCase');
    await api_spec.transferCase(judgeUser);
  }
});
