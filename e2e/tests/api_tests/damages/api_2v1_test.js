...
Feature('CCD 2v1 API test @api-unspec @api-multiparty @api-tests-2v1 @api-prod-unspec @api-sher1');

Scenario('Create claim', async ({I, api}) => {
  await api.createClaimWithRepresentedRespondent(config.secondDefendantSolicitorUser, mpScenario);
});
...
Scenario('Notify claim', async ({I, api}) => {
  await api.notifyClaim(config.secondDefendantSolicitorUser);
});

Scenario('Notify claim details', async ({I, api}) => {
  await api.notifyClaimDetails(config.secondDefendantSolicitorUser);
});
//
...
Scenario('Claimant response', async ({I, api}) => {
  await api.claimantResponse(config.secondDefendantSolicitorUser, mpScenario, 'AWAITING_APPLICANT_INTENTION', 'FOR_SDO', 'FAST_CLAIM');
});
//
