...
Feature('Unspec 1v2lips api journey').tag('@api-cos @api-nightly-prod');

Scenario('01 Create claim where one respondent is LIP one is LR and notify/notify details', async ({api}) => {
...
Scenario('02 Create claim where two respondents are LIP and notify/notify details', async ({api}) => {
  await api.createClaimWithRespondentLitigantInPerson(config.applicantSolicitorUser,
                                                      'ONE_V_TWO_LIPS');
